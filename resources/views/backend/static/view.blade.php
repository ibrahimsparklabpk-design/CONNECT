                ...
                <tr>
                    ...
                    <td>
                        <input type="number" name="quantity" class="form-control quantity-input"
                            ...
                            data-price="{{ ($item['quantity'] ?? 1) > 0 ? ($item['price'] ?? 0) / ($item['quantity'] ?? 1) : 0 }}">
                    </td>
                    ...
                    <td>
                        <form action="{{ route('static.cart.remove', $index) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" style="background:black;color:white;border:none;padding:6px 14px;border-radius:5px;cursor:pointer;">
                                Remove Item
                            </button>
                        </form>
                    </td>
                    ...
                    <td>
                        <form action="{{ route('static.cart.clear') }}" method="POST" style="display:inline;">
                            @csrf
                            <button type="submit" style="background:black;color:white;border:none;padding:6px 14px;border-radius:5px;cursor:pointer;" class="btn btn-sm btn-warning">
                                Clear Cart
                            </button>
                        </form>
                    </td>
